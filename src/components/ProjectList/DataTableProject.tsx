...
import DropdownProjectEdit from "../ProjectID/DropdownProjectEdit";
import CheckboxProjectSelect from "./CheckboxProjectSelect";
import BadgeProjectStatus from "../BadgeProjectStatus/BadgeProjectStatus";
import DropdownProStatus from "../ProjectStatus/DropdownsProStatus";

// 新しいinterface Projectに変更
interface Project {
    name: string;
    targetAmount: string;
...
// サンプルデータもProject型に合わせる
const dataOne: Project[] = [
    {
        name: "プロジェクトA",
        targetAmount: "100000",
        currentAmount: "50000",
...
const DataTableProject = () => {
    ...
    const defaultColumn = useMemo(
        () => ({
            Filter: ColumnFilter as React.FC<FilterProps<Project>>,
        }),
        []
    );

    // プロジェクトステータス管理用のstate（必要に応じて値や表示を変更）
    const [projectStatus, setProjectStatus] = useState("Open");
    const statuses = ["受付中", "受付停止", "登録情報不備"];

    // CSVダウンロード処理（選択行のデータをCSV出力）
            ...
            hooks.visibleColumns.push((cols) => [
                {
                    ...
                    Cell: ({ row }) => (
                        <div className="pl-2">
                            <CheckboxProjectSelect {...row.getToggleRowSelectedProps()} />
                        </div>
                    ),
                },
                ...cols,
